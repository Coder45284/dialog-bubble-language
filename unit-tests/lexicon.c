...
void lexerTestCallback(int token_type, const YYSTYPE *const yystype) {
    ...
    if(expected_results[token_type_index].token_type != token_type) {
        printf("Type wrong expected %s(%d) for word at %i index. %s\n", tokenTypeString(expected_results[token_type_index].token_type), expected_results[token_type_index].token_type, token_type_index, yytext);
        problem = 1;
    }
    if(strcmp(expected_results[token_type_index].string, result) != 0) {
        printf("Return does not match expected %s(%d) for word at %i index. Original Word: %s. Expected: \"%s\". Returned: \"%s\"\n", tokenTypeString(expected_results[token_type_index].token_type), expected_results[token_type_index].token_type, token_type_index, yytext, expected_results[token_type_index].string, result);
        problem = 1;
    }
    token_type_index++;
}
